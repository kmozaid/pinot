...
public class PartitionSegmentPruner implements SegmentPruner {
  ...
  private static final Map<String, PartitionInfo> INVALID_COLUMN_PARTITION_INFO
          = Map.of("null", new PartitionInfo(null, null));
  ...
  public void init(IdealState idealState, ExternalView externalView, Set<String> onlineSegments) {
    ...
    for (int i = 0; i < numSegments; i++) {
      ...
      Map<String, PartitionInfo> columnPartitionInfo = extractPartitionInfoFromSegmentZKMetadataZNRecord(segment, znRecords.get(i));
   ...
   *       {@link #INVALID_COLUMN_PARTITION_INFO} when the segment does not have valid partition metadata in its ZK metadata,
   *       in which case we won't retry later.
   */
  @Nullable
  private Map<String, PartitionInfo> extractPartitionInfoFromSegmentZKMetadataZNRecord(String segment, @Nullable ZNRecord znRecord) {
    if (znRecord == null) {
    ...
    for(String partitionColumn : _partitionColumns) {
      ...
      PartitionInfo partitionInfo = new PartitionInfo(PartitionFunctionFactory.getPartitionFunction(
              columnPartitionMetadata.getFunctionName(), columnPartitionMetadata.getNumPartitions(),
              columnPartitionMetadata.getFunctionConfig()), columnPartitionMetadata.getPartitions());
  ...
  public Set<String> prune(BrokerRequest brokerRequest, Set<String> segments) {
    ...
    if (pinotQuery != null) {
      ...
      for (String segment : segments) {
        ...
        if (columnPartitionInfo == null || columnPartitionInfo == INVALID_COLUMN_PARTITION_INFO
                || isPartitionMatch(filterExpression, columnPartitionInfo)) {
      ...
      for (String segment : segments) {
        Map<String, PartitionInfo> columnPartitionInfo = _segmentColumnPartitionInfoMap.get(segment);
        if (columnPartitionInfo == null || columnPartitionInfo == INVALID_COLUMN_PARTITION_INFO
                ||  isPartitionMatch(filterQueryTree, columnPartitionInfo)) {
          selectedSegments.add(segment);
        }
      }
      return selectedSegments;
